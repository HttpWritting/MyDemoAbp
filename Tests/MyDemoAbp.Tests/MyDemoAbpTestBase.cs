...
namespace MyDemoAbp.Tests
{
    public abstract class MyDemoAbpTestBase : AbpIntegratedTestBase<MyDemoAbpTestModule>
    {
        ...
        protected void UsingDbContext(Action<MyDemoAbpDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MyDemoAbpDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MyDemoAbpDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MyDemoAbpDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MyDemoAbpDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyDemoAbpDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MyDemoAbpDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyDemoAbpDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MyDemoAbpDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MyDemoAbpDbContext, Task<T>> func)
        {
            T result;
